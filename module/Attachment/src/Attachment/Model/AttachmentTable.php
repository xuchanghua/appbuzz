...
class AttachmentTable
{
    ...
    {
        $id  = (int) $id;
        $rowset = $this->tableGateway->select(array('id_attachment' => $id));
        $row = $rowset->current();
        if (!$row) {
    ...
    public function saveAttachment(Attachment $attachment)
    {
        ...
        $id = (int)$attachment->id_attachment;
        if ($id == 0) {
            $this->tableGateway->insert($data);
        } else {
            if ($this->getAttachment($id)) {
                $this->tableGateway->update($data, array('id_attachment' => $id));
            } else {
    ...
    {
        $this->tableGateway->delete(array('id_attachment' => $id));
    }
